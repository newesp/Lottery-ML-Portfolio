...
import { Callout, Eyebrow, Metric, RepoLink, Steps } from "./components/UI";
  ...
  return <>
    <section className="hero section-shell">
      <div><Eyebrow>REPRODUCIBLE ML CASE STUDY · 可重現的機器學習案例</Eyebrow>
        ...
        <p className="hero-copy">這個專案用台灣威力彩示範一套<RepoLink path="docs/system-design.md">完整的機器學習流程</RepoLink>：保存不可任意改動的原始資料備份（immutable snapshot）、確保模型偷看不到未來答案（leakage-safe features）、依時間順序反覆測試（Time Series Cross Validation），最後才打開保留資料做總驗收（locked holdout）。重點不是提供明牌，而是證明實驗過程經得起檢查。</p>
      ...
      <div className="hero-panel" aria-label="專案關鍵結果">
        ...
        <small className="hero-meaning">「平均命中」是選出的 6 個號碼中，平均有幾個與開獎號碼相同；不是中獎率，也不是頭獎機率。</small>
    ...
    <section className="section-shell content-section"><Callout tone="warning" title="最重要的研究結論"><p>在最後才打開的 holdout 測試中，模型的平均命中數看起來比 Uniform（完全隨機選號）高；但 paired bootstrap 95% CI 跨越 0，代表這個差距仍可能只是抽樣運氣。更簡單的 Rolling Frequency（依近期出現頻率排序）甚至略高於模型。因此目前沒有證據能說模型穩定勝過隨機選號；完整結果與限制可在<RepoLink path="docs/model-card.md">模型卡</RepoLink>交叉核對。</p></Callout></section>
  </>;
}
